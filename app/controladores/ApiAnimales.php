...
class ApiAnimales extends Controlador
{
    ...
    public function animales(): void
    {
        ...
        if ($method === 'GET') {
            $cars = $this->modelo->obtenerAnimales();
    ...
    public function car(int $id): void
    {
        ...
        if ($method === 'GET') {
            $car = $this->modelo->obtenerAnimal($id);
            if (!$car) {
                $this->jsonResponse(["error" => "Animal not found"], 404);
            }
        ...
        if ($method === 'DELETE') {
            ...
            $exists = $this->modelo->obtenerAnimal($id);
            if (!$exists) {
                $this->jsonResponse(["error" => "Animal not found"], 404);
            }

            $ok = $this->modelo->deleteAnimal($id);
            if ($ok) {
                $this->jsonResponse(["message" => "Animal deleted"], 200);
            }
